...
from scripts.tokenization import WhitespaceTokenizer
import random
import numpy as np
import torch
import torch.distributed as dist
import json
from utils import seq2seq_loader
from torch.utils.data import RandomSampler, DistributedSampler
from utils.loader_utils import batch_list_to_batch_tensors
from models.modeling import BertForPreTrainingLossMask
import math
from tqdm import tqdm, trange

from utils.optimization import BertAdam, warmup_linear

with open("../configs/config_unilm.yaml", "r") as f:
    config = yaml.safe_load(f)

print(config["training"]["output_dir"])  # Example access
logging.basicConfig(format='%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    datefmt='%m/%d/%Y %H:%M:%S',
                    level=logging.INFO)

# Extract output directory
train_output_dir = config["training"]["output_dir"]
...
# Call it (Liliia)
latest_epoch = _get_max_epoch_model(train_output_dir)
# Call it end (Liliia)

def main():
    global train_dataloader, train_sampler
    parser = argparse.ArgumentParser()

    # Required parameters
    parser.add_argument("--data_dir",
                        default=config["training"]["data_dir"],
                        type=str,
                        # required=True,
                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")
    parser.add_argument("--src_file", config["data"]["train_src"], type=str,
                        help="The input data file name.")
    parser.add_argument("--tgt_file", config["data"]["train_tgt"], type=str,
                        help="The output data file name.")
    # parser.add_argument("--bert_model", default=None, type=str, required=True,
    #                     help="Bert pre-trained model selected in the list: bert-base-uncased, "
    #                          "bert-large-uncased, bert-base-cased, bert-base-multilingual, bert-base-chinese.")
    parser.add_argument("--model_recover_path",
                        default=config["model"]["model_recover_path"],
                        type=str,
                        help="Path to the pretrained UNILM model checkpoint (.bin file)")
    # parser.add_argument("--config_path", default=None, type=str,
    #                     help="Bert config file path.")
    parser.add_argument("--config_path",
                        default=config["model"]["config_path"],
                        type=str,
                        help="Path to model config.json file")
    parser.add_argument("--vocab_path",
                        default=config["model"]["vocab_path"],
                        type=str,
                        help="Path to model vocab.txt file")
    ...
    parser.add_argument("--log_dir",
                        default=config["logging"].get("log_dir", "./logs"),
                        type=str,
    ...
    parser.add_argument("--optim_recover_path",
                        default=config["training"].get("optim_recover_path", None),
                        type=str,
                        help="The file of pretraining optimizer.")
    # Other parameters
    parser.add_argument("--max_seq_length",
                        default=config["tokenizer"]["max_seq_length"],
                        type=int,
                        help="The maximum total input sequence length after WordPiece tokenization. \n"
                             "Sequences longer than this will be truncated, and sequences shorter \n"
                             "than this will be padded.")
    parser.add_argument("--do_train",
                        default=config["training"]["do_train"],
                        type=bool,
                        help="Whether to run training.")
    parser.add_argument("--do_eval",
                        default=config["training"]["do_eval"],
                        type=bool,
                        help="Whether to run eval on the dev set.")
    parser.add_argument("--do_lower_case",
                        default=config["tokenizer"]["do_lower_case"],
                        type=bool,
    ...
    parser.add_argument("--eval_batch_size",
                        default=config["training"]["eval_batch_size"],
                        type=int,
                        help="Total batch size for eval.")
    parser.add_argument("--learning_rate", default=config["training"]["learning_rate"], type=float,
                        help="The initial learning rate for Adam.")
    parser.add_argument("--label_smoothing", config["training"]["label_smoothing"], type=float,
                        help="The initial learning rate for Adam.")
    ...
    parser.add_argument("--finetune_decay",
                        default=config["training"]["finetune_decay"],
                        type=bool,
    ...
    parser.add_argument("--warmup_proportion",
                        default=config["training"]["warmup_proportion"],
                        type=float,
                        help="Proportion of training to perform linear learning rate warmup for. "
                             "E.g., 0.1 = 10%% of training.")
    parser.add_argument("--hidden_dropout_prob", default=config["training"]["hidden_dropout_prob"], type=float,
                        help="Dropout rate for hidden states.")
    parser.add_argument("--attention_probs_dropout_prob", default=config["training"]["attention_probs_dropout_prob"], type=float,
    ...
    parser.add_argument('--seed',
                        type=int,
                        default=config["logging"]["seed"],
                        help="random seed for initialization")
    parser.add_argument('--gradient_accumulation_steps',
                        type=int,
                        default=config["training"]["gradient_accumulation_steps"],
                        help="Number of updates steps to accumulate before performing a backward/update pass.")
    parser.add_argument('--fp16', default=config["training"]["fp16"],
                        type=bool,
                        help="Whether to use 16-bit float precision instead of 32-bit")
    parser.add_argument('--fp32_embedding', default=config["training"]["fp32_embedding"],
                        type=bool,
                        help="Whether to use 32-bit float precision instead of 16-bit for embeddings")
    parser.add_argument('--loss_scale', type=float, default=config["training"]["loss_scale"],
                        help="Loss scaling to improve fp16 numeric stability. Only used when fp16 set to True.\n"
                             "0 (default value): dynamic loss scaling.\n"
                             "Positive power of 2: static loss scaling value.\n")
    parser.add_argument('--amp', default=config["training"]["amp"],
                        type=bool,
                        help="Whether to use amp for fp16")
    parser.add_argument('--from_scratch', default=config["training"]["from_scratch"],
                        type=bool,
                        help="Initialize parameters with random values (i.e., training from scratch).")
    parser.add_argument('--new_segment_ids', default=config["training"]["new_segment_ids"],
                        type=bool,
                        help="Use new segment ids for bi-uni-directional LM.")
    parser.add_argument('--new_pos_ids', default=config["training"]["new_pos_ids"],
                        type=bool,
                        help="Use new position ids for LMs.")
    parser.add_argument('--tokenized_input', default=config["training"]["tokenized_input"],
                        type=bool,
                        help="Whether the input is tokenized.")
    parser.add_argument('--max_len_a', type=int, default=config["training"]["max_len_a"],
                        help="Truncate_config: maximum length of segment A.")
    parser.add_argument('--max_len_b', type=int, default=config["training"]["max_len_b"],
                        help="Truncate_config: maximum length of segment B.")
    parser.add_argument('--trunc_seg', default=config["training"]["trunc_seg"],
                        type=str,
                        help="Truncate_config: first truncate segment A/B (option: a, b).")
    parser.add_argument('--always_truncate_tail', default=config["training"]["always_truncate_tail"],
                        type=bool,
                        help="Truncate_config: Whether we should always truncate tail.")
    parser.add_argument("--mask_prob", default=config["training"]["mask_prob"], type=float,
                        help="Number of prediction is sometimes less than max_pred when sequence is short.")
    parser.add_argument("--mask_prob_eos", default=config["training"]["mask_prob_eos"], type=float,
                        help="Number of prediction is sometimes less than max_pred when sequence is short.")
    parser.add_argument('--max_pred', type=int, default=config["training"]["max_pred"],
                        help="Max tokens of prediction.")
    parser.add_argument("--num_workers", default=config["training"]["num_workers"], type=int,
                        help="Number of workers for the data loader.")
    parser.add_argument('--mask_source_words', default=config["training"]["mask_source_words"],
                        type=bool,
                        help="Whether to mask source words for training")
    parser.add_argument('--skipgram_prb', type=float, default=config["training"]["skipgram_prb"],
                        help='prob of ngram mask')
    parser.add_argument('--skipgram_size', type=int, default=config["training"]["skipgram_size"],
                        help='the max size of ngram mask')
    parser.add_argument('--mask_whole_word', default=config["training"]["mask_whole_word"],
                        type=bool,
                        help="Whether masking a whole word.")
    parser.add_argument('--do_l2r_training', default=config["training"]["do_l2r_training"],
                        type=bool,
                        help="Whether to do left to right training")
    parser.add_argument('--has_sentence_oracle', default=config["training"]["has_sentence_oracle"],
                        type=bool,
                        help="Whether to have sentence level oracle for training. "
                             "Only useful for summary generation")
    parser.add_argument('--max_position_embeddings', type=int, default=config["training"]["max_position_embeddings"],
                        help="max position embeddings")
    parser.add_argument('--relax_projection', default=config["training"]["relax_projection"],
                        type=bool,
                        help="Use different projection layers for tasks.")
    parser.add_argument('--ffn_type', default=config["training"]["ffn_type"], type=int,
                        help="0: default mlp; 1: W((Wx+b) elem_prod x);")
    parser.add_argument('--num_qkv', default=config["training"]["num_qkv"], type=int,
                        help="Number of different <Q,K,V>.")
    parser.add_argument('--seg_emb', default=config["training"]["seg_emb"],
                        type=bool,
                        help="Using segment embedding for self-attention.")
    parser.add_argument('--s2s_special_token', default=config["training"]["s2s_special_token"],
                        type=bool,
                        help="New special tokens ([S2S_SEP]/[S2S_CLS]) of S2S.")
    parser.add_argument('--s2s_add_segment', default=config["training"]["s2s_add_segment"],
                        type=bool,
                        help="Additional segmental for the encoder of S2S.")
    parser.add_argument('--s2s_share_segment', default=config["training"]["s2s_share_segment"],
                        type=bool,
                        help="Sharing segment embeddings for the encoder of S2S (used with --s2s_add_segment).")
    parser.add_argument('--pos_shift', default=config["training"]["pos_shift"],
                        type=bool,
    ...
    with open(os.path.join(args.output_dir, 'opt.json'), 'w') as f:
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        if recover_step:
            ...
            raise ValueError("No model checkpoint provided to recover from.")
        model = BertForPreTrainingLossMask.from_pretrained(
